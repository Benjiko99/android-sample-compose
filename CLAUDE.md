...
- **Theming** lives in `app/src/main/java/uno/lux/sample/ui/theme/`. `SampleTheme` wraps `MaterialTheme` and prefers **dynamic color** (Material You) on Android 12+, falling back to the hand-defined light/dark schemes built from the palette in `Color.kt`; typography is in `Type.kt`. Wrap any new top-level Compose content (and `@Preview`s) in `SampleTheme` so dynamic color and dark mode work. The light/dark choice is **user-controlled**: `MainActivity` collects the `ThemeMode` (Light/Dark/System) from `SettingsRepository`, resolves it with `ThemeMode.isDark(isSystemInDarkTheme())`, and passes the result to `SampleTheme(darkTheme = …)`; the settings screen changes it through `SettingsViewModel`. Edge-to-edge system-bar styling is re-applied in a `DisposableEffect` keyed on the resolved theme so the bar icons keep contrast.

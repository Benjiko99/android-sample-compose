...
- **Entry point** — `MainActivity` (`app/src/main/java/uno/lux/sample/MainActivity.kt`) is the only Activity. In `setContent` it collects the `ThemeMode` preference, resolves the dark/light choice, applies it to `MosaicTheme`, and hosts `SampleApp()`.

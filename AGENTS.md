...
- **Feature architecture — the HOME feed sets the pattern to follow.** Code is layered by responsibility:
  ...
  - `ui/create/` — the post composer behind the CREATE tab, the same stateful-binder + stateless-screen split. `CreatePostViewModel` holds one plain `StateFlow<CreatePostUiState>` (form + in-flight flag + error) rather than a sealed hierarchy, because a composer has nothing to load. Publishing goes through `FeedRepository.publish`, which calls `PostRepository.create` (storing the entity), ingests the embedded author, and prepends the new ID to the feed so the post is visible without a re-fetch; the ViewModel then pushes `Screen.PostDetail`. A failed publish keeps the typed text. Field limits (`CreatePostTitleMaxLength`, `CreatePostBodyMaxLength`) mirror the backend's `Post` validations so errors surface before the request. Media attachments are deliberately not authorable — a `NewPost` is title + body.
